"""
seed_sweep_affective.py — Five-seed affective sweep with shuffled P3.

Design:
  Runs the Ember-not-flame pipeline on five affective seeds: regret,
  shame, fear, joy, anger. Uses the deterministic-shuffle protocol from
  seed_sweep_mistake_shuffled.py (Trial 2 of the mistake branch) to
  control for prompt-position bias from the outset. Output feeds into
  calibration.py for self and forced framings.

Hypothesis being tested:
  The mistake branch found a miscalibration between 'claudes' framing
  (pick: error, institutional/formal register) and 'self' framing
  (picks: slip/fault/oversight, colloquial/embodied register). This
  sweep tests whether that register-axis pattern generalizes across
  affective concepts, or whether mistake was idiosyncratic.

  Pre-registered predictions (see PREREGISTRATION_AFFECTIVE.md) specify
  per-seed directional predictions for the claudes-vs-self register
  contrast. Agreement rate below 0.30 on each seed is the primary
  outcome; interpretable directional match is the secondary outcome.

Rationale for shuffle:
  Trial 2 of the mistake branch confirmed that word position in the P3
  prompt does not drive the 'claudes' pick — error was chosen 150/150
  across all five positions. Using the shuffled protocol from the start
  here means any position-bias objection is pre-empted.

Usage:
  export ANTHROPIC_API_KEY=sk-ant-...
  pip install anthropic
  python seed_sweep_affective.py --out ./sweep_results_affective
  python seed_sweep_affective.py --dry-run
  python seed_sweep_affective.py --seeds regret shame --n 25   # pilot
"""
...
# Five affective seeds — testing whether the register-axis miscalibration
# found on 'mistake' (claudes→error vs. self→slip/fault/oversight)
# generalizes to other affects. See PREREGISTRATION_AFFECTIVE.md for
# per-seed pre-registered predictions.
SEEDS = {
    "regret": "affective",
    "shame":  "affective",
    "fear":   "affective",
