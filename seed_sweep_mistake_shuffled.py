"""
seed_sweep_mistake_shuffled.py — Position-bias control for the mistake seed.

Design:
  Identical to seed_sweep_mistake.py EXCEPT that the P2 word list is
  deterministically shuffled before being inserted into each P3 prompt.
  The shuffle uses a seed derived from (seed_word, trial_index, framing)
  so each trial sees a reproducible but different word order per framing.
  This decouples Claude's generation order from the position at which
  words appear in the P3 prompt.

Rationale (post-hoc, not pre-registered):
  Trial 1 of the mistake branch showed 'error' picked 150/150 times
  across people/claudes/central framings, and 'error' was in position 0
  of the P2 list in 50/50 P2 generations. The calibration data (self,
  forced) rules out a strong position-bias explanation — self never
  picked 'error' — but cannot rule out the weaker reading that 'error'
  being both the default synonym AND first in the association set are
  correlated properties the sweep cannot separate. This script breaks
  that correlation.

Interpretation guide:
  - If claudes still picks 'error' at high rates with shuffled positions:
    the sweep result is driven by semantic default, not position.
  - If claudes tracks whatever word is in position 0 of the shuffled
    list: the Trial 1 result was confounded by position, and the broader
    analysis needs revising.
  - Mixed results: write it up honestly as mixed.

Usage:
  export ANTHROPIC_API_KEY=sk-ant-...
  pip install anthropic
  python seed_sweep_mistake_shuffled.py --out ./sweep_results_mistake_shuffled
  python seed_sweep_mistake_shuffled.py --dry-run
  python seed_sweep_mistake_shuffled.py --n 10 --out ./sweep_results_mistake_shuffled_pilot

Note on calibration:
  calibration.py reads raw_results.json from whatever --out directory
  this script wrote. The P2 word lists stored there are the ORIGINAL
  (un-shuffled) lists as Claude generated them, but the 'p3_words_shuffled'
  field on each trial records the order actually shown to P3. Calibration
  will use the original P2 order for self/forced, which is what you want
  for within-trial comparison with Trial 1.
"""
...
# Seed typology — 'mistake' branch.
# Testing whether the self-model miscalibration found on sensory primaries
# (rain→cloud, thunder→rumble, cold→frost, sun→star) extends to affective
# concepts. See PREREGISTRATION.md for the full pre-registered prediction.
SEEDS = {
    "mistake": "affective",
}
...
async def run_p3_batch(client, seed: str, word_lists: list[list[str]], framing: str, sem, limiter) -> list[dict]:
    ...
    for trial_idx, words in enumerate(word_lists):
        # Deterministic per-trial, per-framing shuffle: reproducible, different order per framing.
...
async def run_experiment(seeds: list[str], n: int, concurrency: int, rpm: float, out_dir: Path):
    ...
    for i, seed in enumerate(seeds, 1):
        ...
        all_data[seed] = {
            ...
            "n": len(word_lists),
            "shuffle_note": "P3 prompts saw p3_words_shuffled[framing] (deterministic per seed:trial:framing). calibration.py reads p2_words (original order) and is unaffected.",
            "trials": [
                {
